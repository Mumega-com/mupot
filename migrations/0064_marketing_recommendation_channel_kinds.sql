-- 0064: widen recommendation kind/KPI CHECKs for SEO/content channel intelligence.
-- Adds content_review kind and content primary_kpi while preserving rows + fences.

PRAGMA foreign_keys = OFF;

CREATE TABLE IF NOT EXISTS marketing_recommendations_new (
  ...
  primary_kpi TEXT NOT NULL CHECK (primary_kpi IN (
    'visibility','qualifiedTraffic','leads','conversion','revenue','content'
  )),
...
INSERT INTO marketing_recommendations_new SELECT * FROM marketing_recommendations;
DROP TABLE marketing_recommendations;
ALTER TABLE marketing_recommendations_new RENAME TO marketing_recommendations;
...
-- Widen observation metric/authority CHECKs for keyword-gap + content_surface metrics.

DROP TRIGGER IF EXISTS marketing_monitor_runs_finalize_fence;
DROP TRIGGER IF EXISTS marketing_monitor_observations_insert_fence;
DROP TRIGGER IF EXISTS marketing_monitor_observations_no_update;
DROP TRIGGER IF EXISTS marketing_monitor_observations_no_delete;

CREATE TABLE IF NOT EXISTS marketing_monitor_observations_new (
  run_id TEXT NOT NULL,
  tenant TEXT NOT NULL,
  installation_id TEXT NOT NULL,
  binding_generation_id TEXT NOT NULL,
  ...
  CHECK (
    (metric_key IN (
      'seo.ai_citations','seo.organic_sessions','growth.leads','growth.replies',
      'seo.keyword_gap_queries','content.posts_published'
    ) AND unit = 'count')
  ...
  CHECK (
    ...
    OR (metric_key = 'seo.keyword_gap_queries' AND authority IN ('first-party','gsc'))
    OR (metric_key = 'finance.revenue' AND authority IN ('ghl','crm'))
    OR (metric_key = 'content.posts_published' AND authority IN ('mcpwp','inkwell'))
  )
);

INSERT INTO marketing_monitor_observations_new SELECT * FROM marketing_monitor_observations;
DROP TABLE marketing_monitor_observations;
ALTER TABLE marketing_monitor_observations_new RENAME TO marketing_monitor_observations;
