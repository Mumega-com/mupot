...
describe('squad mupot cutover runbook', () => {
  ...
  it('keeps SOS wake-hook removal gated by the complete host evidence bundle', () => {
    ...
    expect(runbook).toContain('--host-go-plan')
    expect(runbook).toContain('npm run receipt:bundle:plan')
    expect(pkg.scripts['receipt:bundle:plan']).toBe('node fleet-runtime/receipt-bundle.mjs --host-go-plan')
    ...
    expect(runbook).toContain('exported sidecars are portable')
    ...
    expect(runbook).toContain('recomputes their')
    expect(runbook).toContain('status/summary from their own checks')
    expect(runbook).toContain('rejects secret material')
    expect(runbook).toContain('non-self-contained copied evidence')
