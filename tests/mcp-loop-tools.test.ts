...
describe('loop_set_status — promote paused → active', () => {
  ...
  it('promotes a paused loop to active', async () => {
    const db = makeDb()
    const id = 'loop-paused-1'
    const inserted = await insertLoopIfAbsent(db.env, id, 'paused', VALID_SPEC)
  ...
  it('refuses to revive a killed loop', async () => {
    const db = makeDb()
    const id = 'loop-killed-1'
    await insertLoopIfAbsent(db.env, id, 'paused', VALID_SPEC)
