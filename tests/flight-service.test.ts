            ...
            return {
              async run() {
                if (sql.includes('INSERT INTO flights')) {
                  const [id, t, projectId, agent, dispatchedBy, goal, trig, budget, meta] = a as [
                    string, string, string | null, string, string, string, string, number | null, string,
                  ]
                  rows.set(id, {
                    id, tenant: t, project_id: projectId, agent, dispatched_by_agent_id: dispatchedBy, goal, status: 'preflight', trigger_source: trig as FlightRow['trigger_source'],
