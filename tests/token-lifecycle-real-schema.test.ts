...
describe('every bearer door consumes the shared predicate', () => {
  ...
  const NON_AUTH_QUERIES: ReadonlyArray<{ fingerprint: string; why: string }> = [
    ...
    { fingerprint: '05aee212eb22', why: 'pots/service.ts readFullSeedIdentityState — admin-token existence check for provisionSovereignPot\'s idempotent-retry gate (mupot#1507 P0-2). Answers "was this pot already seeded," not "may this bearer authenticate": an EXPIRED admin token still proves seeding completed, so applying TOKEN_LIVE_PREDICATE\'s expiry check here would misreport a successfully-seeded-but-aged-out pot as partially seeded and try to reseed it. Filters revoked_at IS NULL only (a revoked token means the credential was deliberately killed, not that seeding failed) and grants nothing — it runs against the freshly-created CHILD pot\'s own D1 via the D1 REST API, never against this Worker\'s own auth path.' },
    { fingerprint: 'bba50cbd88a9', why: 'pots/service.ts readFullSeedIdentityState — the lead agent\'s seed-seat token existence check, same idempotent-retry gate and same reasoning as the admin-token check immediately above (fingerprint 05aee212eb22).' },
  ]
