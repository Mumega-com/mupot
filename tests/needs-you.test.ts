...
function makeHarness(options: { includeRoutineMigrations?: boolean } = {}): SqliteD1Harness {
  ...
  for (const file of readdirSync(MIGRATIONS_DIR).filter(name => name.endsWith('.sql')).sort()) {
    if (
      ...
      // its kind CHECK to admit 'project_access') — it depends on 0073's
      // table existing, so it belongs in the SAME "routine tables absent"
      // exclusion group as 0073/0074, or this rolling-deploy simulation
      // breaks with "no such table: routine_run_actions" instead of
      // exercising the degrade-to-empty path this test is actually about.
