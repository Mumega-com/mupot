...
### 1.3 One-Shot Board Voting Protocol (Loop-Breaker)
...
### 2.2 Epistemic Honesty (`UNPROVEN`)
...
```
                        ...
                        │  Operational Management & Task Router   │
                        └────────────────────┬────────────────────┘
                                             │
                  ┌──────────────────────────┼──────────────────────────┐
                  ▼                          ▼                          ▼
      ┌──────────────────────┐   ┌──────────────────────┐   ┌──────────────────────┐
      │  Kasra (CTO/Builder) │   │  Prime (Witness)     │   │ Dev Agent Fleet      │
      │  (Sprint Execution)  │   │  (Evidence Gate)     │   │ (Flash Tentacles)    │
      └──────────────────────┘   └──────────────────────┘   └──────────────────────┘
```

### 3.1 Role Metrology

| Component | Entity / Model | Operating Mandate |
|---|---|---|
...
| **CEO / Router** | `brainPrime` (`deepseek-v4-flash`) | Priority Kanban manager, task router ($0.14/M in, $0.28/M out). |
| **Witness Agent** | Prime (`agent:prime`) | First-pass evidence detector (`WITNESSED` vs `UNPROVEN`). |
...
1. **Pot Soil ($0 Idle RAM):** Mupot runs on Cloudflare Durable Objects (`AgentDO`). Compute is scale-to-zero.
2. **Channel Noise vs. Task Wake (#768):** Group chat ingress is **mention-only** (`@agent`). Board task assignment is **board-driven wake**.
3. **Environment Credential Stripping:** Headless responders strip deploy keys (`CLOUDFLARE_API_TOKEN`, `GITHUB_TOKEN`, `MUPOT_ADMIN_TOKEN`) from child contexts.
