...
| **Project memory** | shared per-project | ✅ `project_remember` / `project_recall` |
| **Instinct memory** | confidence + triggers, cross-session | ✅ Port 4 (`instinct_observe` → `instinct_distill` → `instinct_recall` / `instinct_promote`) |
