...
1. Calls `closeGitHubPrMirrorTasks(env, { htmlUrl, repo, prNumber })` — marks
   matching **ungated** `open`/`in_progress` mirrors `done` (match by PR
   `html_url` in body and/or `[GH repo] PR #N` title prefix).
...
4. Ignores `synchronize` / `edited` / other non-open actions for task minting
   (`GH_PR_TASK_CREATE_ACTIONS` = `opened` | `reopened` only).
