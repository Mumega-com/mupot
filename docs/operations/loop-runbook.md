...
1. **tech-grok-worker.py** — Grok 4.5, Cursor CLI harness. Minted 2026-08-03 per PR #641 as a distinct executor identity separate from Athena (the coordinator). Dispatches via `cursor-agent -p` in isolated worktrees. Task flow: claim → isolate worktree → dispatch → verify (tsc) → push+PR → move to review → notify.

2. **mumcp-worker.py** — Claude Sonnet (matches `mumcp-agent.service`). WordPress/Elementor automation on the separate MCPWP squad. Dispatches via `claude -p` in the mumcp project directory. WordPress writes are server-forced DRAFT; never publishes. Same task flow as tech-grok.
...
4. **claude-worker.py** — Claude Haiku (cheap execution lane). Executes the majority of build work; expensive Kasra session reserved for gating and decisions. Same task flow: claim → isolate → dispatch → verify → push+PR → review.

**Gate lane (runs after build lanes):**

5. **review-worker.py** — Adversarial review gate. Claude Opus (default), zero tools, isolated scratch cwd, zero MCP/auto-memory/skills. Polls tasks with status=review + gate_owner=gate:kasra-core + a PR link. Never merges by default (REVIEW_AUTOMERGE=0). Returns a verdict receipt; Kasra-core task_verdict finalizes. Dedupes at PR head SHA to avoid re-reviewing unchanged PRs.
...
Each build driver (tech-grok, claude, mumcp) follows the same task lifecycle:

1. **Poll** (`task_list` status=open, assignee_agent_id): fetch open tasks for this driver.
2. **Claim** (`task_update` status=in_progress): atomic; prevents double-dispatch.
3. **Isolate**: create git worktree from main branch.
4. **Dispatch**: run the model (cursor-agent, claude, etc.) in the worktree with the task brief.
5. **Verify**: model must commit real work; run tsc and tests; docs-only diffs skip tsc.
6. **Deliver**: driver (trusted, not the model) pushes branch and opens PR.
7. **Report**: update task status=review, set gate_owner=gate:kasra-core, link PR in body.
8. **Notify**: send mupot inbox message to Kasra-core.
9. **Cleanup**: remove worktree.

### Review Gate Flow

Review-worker runs on each cycle, after all build drivers:

1. **Poll**: task_list status=review, gate_owner=gate:kasra-core, filter to those with "PR: <url>" in body.
2. **Dedupe**: check if PR's current head SHA is already in local reviewed state (~/.fleet/state/review-worker-reviewed.json). If yes, skip.
3. **Classify**: fetch PR's changed files; sensitive=true unless all files match the safe allow-list (docs/, tests/, *.md, README, CHANGELOG, LICENSE).
4. **Review**: run claude in isolated scratch cwd with zero tools. Model hunts P0/P1/WARN findings.
5. **Act**: append receipt to task body; update local dedupe state.
6. **Auto-merge** (REVIEW_AUTOMERGE=1 only, disabled by default): only if verdict=GREEN + no P0/P1 + sensitive=false + repo is canonical Mumega-com/mupot. Re-fetch PR state at merge time, then `gh pr merge`.
7. **Notify**: best-effort inbox ping to Kasra-core.
...
**Symptom:** Every claude/codex/tech-grok task fails at verify with `"This is not the tsc command you are looking for"`.
...
**Fix:** Drivers call `_link_node_modules()` to symlink the repo's node_modules into each worktree before running tsc (scripts/claude-worker.py:109–117, scripts/tech-grok-worker.py:109–117). If this fails:

```bash
cd /home/mumega/mupot-worktrees
rm -f claude-task-*/node_modules codex-task-*/node_modules
...
# Should be: node_modules -> /home/mumega/mupot/node_modules
```
...
**Root cause:** The mupot endpoint rejects oversized `task_update` calls (PR #630). A lost receipt means the task is re-processed every cycle (inefficient, not a blocker).

**Fix:** Drivers cap the task body before updating using `_cap_body()` (scripts/claude-worker.py:120–143, scripts/tech-grok-worker.py:120–143):
...
**Manual intervention if a task body overflows:**
- Fetch the task via mupot MCP.
- Trim the oldest receipts, keep the head and newest tail.
- Re-create the task or wait for steward to reissue.
...
**Steward's auto-detection (PR #650):**
- Polls tasks with status=in_progress.
- Checks if a corresponding worktree exists under `WORKTREE_ROOT`.
- If the task's `updated_at` is older than `ORPHAN_HOURS` (default 2 hours), it's orphaned.
...
**Root cause:** The `--sandbox danger-full-access` flag in codex-worker.py requires `bwrap` (Bubblewrap) to be installed and configured. On the Hetzner host, it may not be available or the user lacks permissions.
...
3. **Prepare tokens:**
   Each driver reads a token file. Tokens are minted via the mupot dashboard or MCP (task `mint_agent_token`). Ensure they have the correct squad scope and capability grants:
   - `~/.fleet/agents/kasra-agent.token` — claude-worker, steward-worker
   - `~/.fleet/agents/kasra-member.token` — review-worker (gate:kasra-core member grant)
   - `~/.fleet/agents/tech-grok-member.token` — tech-grok-worker
   - `~/.fleet/agents/mumega-mumcp-member.token` — mumcp-worker
...
**Orphan detection and reissue:**
```
[2026-08-03T10:15:00Z] steward: found orphaned task <id>: in_progress > 2h, no worktree — reissuing.
```
