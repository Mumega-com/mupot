...
while [ "$STOP" -eq 0 ]; do
  ...
  # deployed fix that was inert. The system had observability without noticing.
  # This ranks anomalies into one digest per cycle and changes nothing; a loop
  # that also acts becomes the act-loop that spammed this fleet before.
  run_driver "gatherer" "$REPO/scripts/gatherer.py"
