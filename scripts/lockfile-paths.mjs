 ...
 */
function declaredDeps(record, isRoot) {
  const out = new Map()
  ...
  if (isRoot) for (const name of Object.keys(record?.devDependencies ?? {})) add(name, false)
...
export function buildGraph(lock) {
  ...
  for (const [path, record] of Object.entries(packages)) {
    if (record?.link) continue // workspace symlink; its real record is the resolved target
    const targets = []
    for (const [name, optional] of declaredDeps(record, path === '')) {
