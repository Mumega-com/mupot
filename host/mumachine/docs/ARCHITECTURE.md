...
`BootSnapshot` holds the returned agent, squad, tenant, channel, brief, roster, and identity-verification outcome. It does not contain a runtime lease, desktop route, quota bucket, or proof of message handling. See [model.rs](../src/model.rs).
