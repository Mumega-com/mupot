...
Current controls are defined in ui.rs (`host/mumachine/src/ui.rs`, local source reference); identity and approval behavior are in client.rs (`host/mumachine/src/client.rs`, local source reference) and device.rs (`host/mumachine/src/device.rs`, local source reference).
...
These options select presentation or safe demo behavior; none enables a receiver, changes authorization, or starts another application. The minimum window size is 880 × 640. See main.rs (`host/mumachine/src/main.rs`, local source reference) for the actual parser.
