...
The 31 tests in [core tests](../src/tests.rs) cover:
...
The 9 [UI tests](../src/ui.rs) cover fencing/cancel/Forget/origin change, expiry, demo isolation, truthful labels, stale refresh, Switch Agent, and form accessibility. Two compile-fail examples prevent serialization of [credentials](../src/model.rs) and [poll context](../src/device.rs).
...
For docs-only review, inspect the diff, resolve relative links, scan for credentials/private IDs, and compare [README](../README.md), [source](../src/lib.rs), and [verification limits](../VERIFICATION.md). This does not refresh Rust/bundle evidence.
