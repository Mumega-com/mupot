...
| --- | --- | --- | --- |
| F-001 | Plain HTTPS Mupot origin validation | Implemented — [model](../src/model.rs) | Reject userinfo, query, fragment, paths and insecure production origins |
| F-002 | Exact existing-agent UUID selection | Implemented — [client](../src/client.rs) | No ambiguous name-based enrollment or agent creation |
| F-003 | Browser-approved device flow | Implemented — [device](../src/device.rs) | Pending/slow-down/denied/expired/cancel states; same-origin approval |
| F-004 | Returned identity/tenant/expiry validation | Implemented — [client](../src/client.rs) | Token response alone is not successful onboarding |
| F-005 | App-owned protected credential storage | Implemented — [vault](../src/vault.rs) | macOS Keychain; unsupported platforms refuse secure storage |
| F-006 | Private profile metadata and transaction locking | Implemented — [profiles](../src/profiles.rs) | Metadata contains no credential; normal failures compensated |
| F-007 | Restore, switch and forget connections | Implemented — [UI](../src/ui.rs) | Forget is local removal, not server revocation |
| F-008 | Boot/orient and accessible roster display | Implemented — [client](../src/client.rs) | Registry context, not runtime launch or idle receive proof |
| F-009 | Explicit app check-in after revalidation | Implemented — [client](../src/client.rs) | Reports this app's seat/harness-unknown metadata only |
| F-010 | Bounded Herdr agent-list discovery | Implemented — [discovery](../src/discovery.rs) | Name/kind/state, not a verified desktop-session binding |
| F-011 | Installed application bundle discovery | Implemented — [discovery](../src/discovery.rs) | Installation does not imply running, connected or supported control |
| F-012 | Native four-page UI, themes, text scaling and labels | Implemented — [UI](../src/ui.rs) | Accessibility tests and historical visual checks are scoped evidence |
| F-013 | Offline labeled demo and launch options | Implemented — [main](../src/main.rs) | Demo disables live network, discovery, profile and Keychain access |
| F-014 | Cancellation and stale-result fences | Implemented — [device](../src/device.rs), [UI](../src/ui.rs) | Late responses cannot change the current operation |
| F-015 | Local macOS development bundle and ZIP | Implemented — [bundle script](../scripts/bundle-macos.sh) | Ad-hoc signature; no installer, automatic launch or notarization |
