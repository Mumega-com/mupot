...
| --- | --- | --- |
| Cargo manifest (`host/mumachine/Cargo.toml`, local source reference), lockfile (`host/mumachine/Cargo.lock`, local source reference), bundle metadata (`host/mumachine/assets/Info.plist`, local source reference) | Actual application/dependency/build declarations | Current 0.1.0 source; platform minimum is not a compatibility test |
| Rust source (`host/mumachine/src/lib.rs`, local source reference) and modules linked in [Architecture](ARCHITECTURE.md) | Implemented behavior | Primary for code claims |
