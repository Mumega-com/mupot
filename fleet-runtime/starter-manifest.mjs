...
export function renderStarterPlan(rawManifest, opts = {}) {
  ...
  for (const agent of agents) {
    ...
    lines.push(
      ...
      commandLine(['node', 'fleet-runtime/control-receipt.mjs', '--observe-state', '--probe-receipt', `${outDir}/probe-start.json`, '--verb', 'start'], ` > ${shellQuote(`${outDir}/control-start.json`)}`),
      commandLine([...startBundle, '--install-receipt', '~/.fleet/receipts/install.json', '--probe-receipt', `${outDir}/probe-start.json`, '--skip-host', '--skip-control']),
      ...
      commandLine(['node', 'fleet-runtime/control-receipt.mjs', '--observe-state', '--probe-receipt', `${outDir}/probe-stop.json`, '--verb', 'stop'], ` > ${shellQuote(`${outDir}/control-stop.json`)}`),
      commandLine([...finalBundle, '--probe-receipt', `${outDir}/probe-stop.json`, '--skip-host', '--skip-runtime', '--skip-control']),
