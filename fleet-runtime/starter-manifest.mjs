...
export function renderStarterPlan(rawManifest, opts = {}) {
  ...
  const lines = [
    ...
    '',
    '1. Install the runtime layout without replacing operator-owned configuration:',
    `node fleet-runtime/install.mjs --service-manager ${manager}`,
    '',
    ...
    `node fleet-runtime/trust-bootstrap.mjs --base-url ${manifest.base_url}`,
    '',
    '3. Activate user services and collect current service status:',
    `node fleet-runtime/service-manager.mjs install --service-manager ${manager}`,
    `node fleet-runtime/service-manager.mjs status --service-manager ${manager} > ~/.fleet/receipts/service.json`,
    '',
    '4. Collect Host-Go and continuous evidence:',
    `node fleet-runtime/host-receipt.mjs --require-services --service-manager ${manager} > ~/.fleet/receipts/host.json`,
  ]

  for (const agent of agents) {
    lines.push(`node fleet-runtime/runtime-receipt.mjs --agent ${agent.agent_id} > ~/.fleet/receipts/runtime-${agent.agent_id}.json`)
    lines.push(`node fleet-runtime/continuous-runtime-receipt.mjs --agent ${agent.agent_id} --service-manager ${manager} --require-control start --require-control stop > ~/.fleet/receipts/continuous-${agent.agent_id}.json`)
  }

  lines.push(
    '',
    '5. Build and verify the attachable evidence bundle:',
    'node fleet-runtime/receipt-bundle.mjs --export --out-dir ~/.fleet/receipts/bundle --export-dir ~/.fleet/receipts/starter-copy',
    'node fleet-runtime/receipt-bundle.mjs --check-manifest --out-dir ~/.fleet/receipts/starter-copy',
    '',
    '6. Data-preserving rollback:',
    `node fleet-runtime/service-manager.mjs uninstall --service-manager ${manager}`,
    '',
    '7. Recovery reinstall and fresh advancement proof:',
    `node fleet-runtime/install.mjs --activate --service-manager ${manager}`,
    ...agents.map((agent) => `node fleet-runtime/continuous-runtime-receipt.mjs --agent ${agent.agent_id} --service-manager ${manager} --require-control start --require-control stop`),
  )
  return `${lines.join('\n')}\n`
}
...
export function verifyStarterBundle(opts = {}) {
  ...
  return validateStarterReceipt(receipt)
}
...
export function main(argv = process.argv.slice(2)) {
  ...
  if (opts.mode === 'plan') process.stdout.write(renderStarterPlan(readManifestForCli(opts.manifestPath), { agents: opts.agents }))
  if (opts.mode === 'verify') process.stdout.write(`${JSON.stringify(verifyStarterBundle({ bundleDir: opts.bundleDir, manifestPath: opts.manifestPath, artifacts: opts.artifacts }), null, 2)}\n`)
  return 0
}
