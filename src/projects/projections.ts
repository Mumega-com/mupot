...
export async function listProjectActivity(
  ...
  const [taskRows, messageRows, flightRows, linkRows, routineEventRows] = await Promise.all([
    ...
    env.DB.prepare(
      `SELECT id, remote_pot, remote_project_id, remote_agent_id, state,
        ...
        WHERE tenant = ?1 AND local_project_id = ?2
          ...
          ${linkAfter.sql}
        ORDER BY ${linkOccurredAt} DESC, id ASC LIMIT ?${linkAfter.nextParam}`,
    ...
    !includeRoutineEvents ? Promise.resolve({ results: [] as Array<{
  ...
  const rows: ProjectProjectionRow<ProjectActivitySource>[] = [
    ...
    ...(linkRows.results ?? []).map((row) => {
      ...
      return {
        ...
        source_id: row.id,
        occurred_at: iso(row.latest_event_at),
...
export async function listProjectEvidence(
  ...
  const [routineOutcomes, routineActions] = includeRoutineEvidence
    ? await Promise.all([
      env.DB.prepare(
      ...
      ).bind(env.TENANT_SLUG, input.projectId, isAdmin, ids, ...routineOutcomeAfter.binds, sourceLimit).all<RoutineOutcomeRow>(),
      env.DB.prepare(
      ...
      ).bind(env.TENANT_SLUG, input.projectId, isAdmin, ids, ...routineActionAfter.binds, sourceLimit).all<RoutineActionRow>(),
    ])
    : [
      { results: [] as RoutineOutcomeRow[] },
      { results: [] as RoutineActionRow[] },
    ]
