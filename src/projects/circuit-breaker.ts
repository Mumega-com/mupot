...
/**
 * Breaker evaluates at/after the boundary, OR early when stalled=1 with a
 * scheduled boundary (slice 4). completed/archived/review are exempt.
 */
export function shouldEvaluateBreaker(
  ...
  if (stalled === 1 && cycleBoundaryAt !== null && cycleBoundaryAt.trim() !== '') {
    return canonicalizeUtcIso(cycleBoundaryAt) !== null
  }
 ...
 * else kill receipt + archive. Re-reads the winning immutable receipt before
 * archive so a mid-sequence recommit wins the UNIQUE race (TOCTOU). Stall never
 * auto-fixes — it only advances this check.
 */
export async function evaluateProjectCircuitBreaker(
  ...
  if (!shouldEvaluateBreaker(project.status, boundaryAt, nowIso, project.stalled)) {
    return 'skipped'
  }
