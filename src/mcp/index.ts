...
const toolCheckIn: ToolSpec = {
  ...
  min: 'authenticated',
  args: `{ seat?: string, harness?: ${SEVEN_AXIS_HARNESSES.map((h) => JSON.stringify(h)).join('|')}, machine?: string, model?: string, provider?: string, effort?: "low"|"medium"|"high"|"extended-thinking-64k", flight_id?: string, source?: string, label?: string, name?: string }`,
  inputSchema: {
    type: 'object',
    properties: {
