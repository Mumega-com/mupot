// mupot — click-to-approve device grant (RFC 8628-shaped).
//
// A harness (Grok Bot, CLI) shows a short user_code. The human opens /device
// (dashboard Google session), matches the code by eye, and clicks Allow or Deny.
// No OTP is typed. No raw token is ever rendered in HTML.
//
// Allow is the same org-trust act as dashboard mint_agent_token /oauth/consent:
// memberMayConsentToAgent (admin-on-squad) or org owner/admin, then
// mintAgentBoundToken. Poll returns the raw once.

import type { AuthContext, Env } from '../types'
import { memberMayConsentToAgent } from '../mcp/oauth-authorize'
import { mintAgentBoundToken, type AgentForMint } from '../members/service'
...
export interface PollDeviceGrantResult {
  status: 'authorization_pending' | 'slow_down' | 'access_denied' | 'expired_token' | 'ok'
  access_token?: string
  token_type?: 'Bearer'
  agent_id?: string
  agent_slug?: string
...
function grantKey(deviceCode: string): string {
  return `${GRANT_PREFIX}${deviceCode}`
}
...
async function readGrant(env: Env, deviceCode: string): Promise<DeviceGrantRecord | null> {
  const row = await env.SESSIONS.get<DeviceGrantRecord>(grantKey(deviceCode), 'json')
  return row ?? null
}

async function writeGrant(env: Env, grant: DeviceGrantRecord): Promise<void> {
  const ttl = Math.max(60, Math.floor((Date.parse(grant.expires_at) - Date.now()) / 1000))
  await env.SESSIONS.put(grantKey(grant.device_code), JSON.stringify(grant), {
    expirationTtl: ttl,
  })
}
...
async function writePending(env: Env, tenant: string, codes: string[]): Promise<void> {
  await env.SESSIONS.put(pendingKey(tenant), JSON.stringify(codes), {
...
export async function createDeviceGrant(
  ...
  for (const code of pending) {
    const g = await readGrant(env, code)
    if (g && g.status === 'pending' && Date.parse(g.expires_at) > Date.now()) live.push(code)
  }
  if (live.length >= DEVICE_PENDING_CAP) return { ok: false, error: 'pending_cap' }

  const deviceCode = randomAlphabet(32)
  ...
  await env.SESSIONS.put(userKey(env.TENANT_SLUG, userCode), deviceCode, {
    expirationTtl: DEVICE_GRANT_TTL_SECONDS,
  })
  live.push(deviceCode)
...
export async function listPendingDeviceGrants(env: Env): Promise<DeviceGrantRecord[]> {
  const codes = await readPending(env, env.TENANT_SLUG)
  const out: DeviceGrantRecord[] = []
  const keep: string[] = []
  for (const code of codes) {
    const g = await readGrant(env, code)
    if (!g) continue
    if (Date.parse(g.expires_at) <= Date.now()) continue
    if (g.status === 'consumed' || g.status === 'denied') continue
    keep.push(code)
    out.push({ ...g, raw_token: null })
  }
  await writePending(env, env.TENANT_SLUG, keep)
  return out
}
...
export async function decideDeviceGrant(
  ...
  const deviceCode = await env.SESSIONS.get(userKey(env.TENANT_SLUG, input.user_code))
  if (!deviceCode) return { ok: false, error: 'not_found' }
  const grant = await readGrant(env, deviceCode)
  ...
  if (input.auth.role !== 'owner' && input.auth.role !== 'admin' && !eligible) {
...
export async function pollDeviceGrant(
  ...
  if (!deviceCode || deviceCode.length > 80) return { status: 'expired_token' }
  const grant = await readGrant(env, deviceCode)
  ...
  if (grant.status === 'denied') return { status: 'access_denied' }
  if (grant.status === 'consumed') return { status: 'expired_token' }
  if (grant.status !== 'approved' || !grant.raw_token) return { status: 'expired_token' }

  const raw = grant.raw_token
  grant.raw_token = null
